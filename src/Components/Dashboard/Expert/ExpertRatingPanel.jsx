...
const ExpertRatingPanel = ({ rating = 5.0, projectCount = 0, userId }) => {
  const navigate = useNavigate()

  return (
    <section className="admin-panel-card expert-rating-panel">
      <div className="d-flex justify-content-between align-items-center mb-2">
        <div className="stat-title mb-0">Expert Rating</div>
        {userId && (
          <span
            className="panel-link text-primary text-decoration-none"
            style={{ fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
